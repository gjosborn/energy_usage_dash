...
def sum_over_hour(dataframe):
    ...
    # Replace sum values for non-latest entries with NaN or None
    merged_df.loc[~latest_entry_mask, 'Value_sum'] = None

    return merged_df


def filter_results(dataframe, min_value, max_value, filter_column):
    """
...
if __name__ == "__main__":
    xml_file_path = 'energy_usage.xml'
    dataframe = parse_xml_to_dataframe(xml_file_path)
    # print(dataframe.head())

    summed_dataframe = sum_over_hour(dataframe)
    # print(summed_dataframe.head())

    summed_dataframe = add_time_period(summed_dataframe)
    # print(summed_dataframe.head())
    print(summed_dataframe.columns)
